...
3. When prompted, set the environment variables: `OPENAI_API_KEY`, `PINECONE_API_KEY`, `PINECONE_INDEX`, `PINECONE_HOST`, `SERVICE_API_KEY`, `LOG_LEVEL` (optional), and `PORT` (optional—defaults to `8080`).
4. Deploy the Blueprint. Render will run `npm ci && npm run build` and start the app with `node dist/server.js`.
5. After deploy, verify the health check:
   ```bash
   curl https://<your-render-service>.onrender.com/health
   ```
6. Test the upsert endpoint:
   ```bash
