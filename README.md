...
- The Profile Domain Capsule only contains subject-matter nouns found in the request (frameworks, specialties, regulations, instruments, etc.) and intentionally excludes AI/LLM labeling terminology.
- The Profile Task Capsule is populated exclusively when explicit AI/LLM data-labeling, model-training, or evaluation evidence is supplied. The service uses a deterministic allowlist (NER, bbox, Label Studio, transcription, RLHF, SFT, etc.) and a validator that blocks generic duties such as analytics, documentation, or EHR workflows.
